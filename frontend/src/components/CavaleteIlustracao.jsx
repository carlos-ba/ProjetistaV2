...
/**
 * Diagrama SVG dinâmico do cavalete de componentes.
 * Recebe dados reais do projeto via props.
 */
export default function CavaleteIlustracao({ condensadora, evaporador, configuracoesMontagem, cavaleteResult, temTanque }) {
  ...
  // Bitolas dos equipamentos — fallback para exibição quando não há dados
  const UC = {
    modelo:      condensadora?.modelo      || '—',
    fabricante:  condensadora?.fabricante  || '',
    cap:         condensadora?.capacidade_kcalh ? `${Number(condensadora.capacidade_kcalh).toLocaleString('pt-BR')} kcal/h` : '',
  ...
  const EV = {
    ...
    cap:         evaporador?.capacidade_kcalh ? `${Number(evaporador.capacidade_kcalh).toLocaleString('pt-BR')} kcal/h` : '',
    liq:         evaporador?.conexao_liquido  || '—',
    suc:         evaporador?.conexao_succao   || '—',
  };

  // Bitolas das linhas do cavalete
  ...
  useEffect(() => {
    ...
    const T = (x, y, s, sz = 9, fill = '#94a3b8', bold = false, anchor = 'middle') => {
      const e = mk('text', { x, y, 'text-anchor': anchor, 'font-size': sz, fill, 'font-weight': bold ? '800' : '500', 'font-family': 'Segoe UI,system-ui,sans-serif' });
      e.textContent = s;
      return e;
    };
    const norm = b => (b || '').replace(/[""]/g, '"').trim();
    const diff = (a, b) => norm(a) !== norm(b);

    // Layout
    const Y_LIQ = 100, Y_SUC = 225, Y_SIF = 318, Y_CSF = 158, R = 11;
    ...
    // ── UC ──────────────────────────────────────────────────────────────
    const drawUC = () => {
      const x = 8, y = 18, w = 86, h = 270;
      mk('rect', { x, y, width: w, height: h, rx: 8, fill: '#071422', stroke: '#0ea5e9', 'stroke-width': 2.5 });
      mk('rect', { x: x + 4, y: y + 4, width: w - 8, height: 82, rx: 5, fill: '#050f1c', stroke: '#0a2030', 'stroke-width': 1 });
      for (let i = 0; i < 5; i++) mk('line', { x1: x + 6, y1: y + 16 + i * 13, x2: x + w - 6, y2: y + 16 + i * 13, stroke: '#0d2540', 'stroke-width': 1.8 });
      const fx = x + w / 2, fy = y + 46;
      ...
      mk('rect', { x: x + 4, y: y + 89, width: w - 8, height: 48, rx: 3, fill: '#050f1c', stroke: '#0a2030', 'stroke-width': 1 });
      for (let i = 0; i < 6; i++) mk('line', { x1: x + 6, y1: y + 97 + i * 7, x2: x + w - 6, y2: y + 97 + i * 7, stroke: '#122545', 'stroke-width': 1.2 });
      mk('rect', { x: x + 4, y: y + 141, width: w - 8, height: 70, rx: 3, fill: '#050f1c', stroke: '#0a2030', 'stroke-width': 1 });
      mk('ellipse', { cx: x + w / 2, cy: y + 178, rx: 20, ry: 24, fill: '#030b16', stroke: '#1e293b', 'stroke-width': 2 });
      mk('ellipse', { cx: x + w / 2, cy: y + 173, rx: 13, ry: 16, fill: '#09162a', stroke: '#334155', 'stroke-width': 1.5 });
      T(x + w / 2, y + 177, 'COMP', 7, '#4a5568', true);
      // Placa de dados
      mk('rect', { x: x + 4, y: y + 216, width: w - 8, height: 49, rx: 4, fill: '#071e38', stroke: '#0ea5e9', 'stroke-width': 1 });
      T(x + w / 2, y + 227, UC.fabricante, 6.5, '#94a3b8');
      T(x + w / 2, y + 238, UC.modelo, 8, '#38bdf8', true);
      T(x + w / 2, y + 249, UC.cap, 6, '#475569');
      // Divisor bitolas
      mk('line', { x1: x + 4, y1: y + 254, x2: x + w - 4, y2: y + 254, stroke: '#0a2545', 'stroke-width': 1 });
      // Líquido
      const liqLabel = `Liq ${UC.liq}`;
      T(x + w / 2, y + 262, liqLabel, 7, '#60a5fa', true);
      // Sucção (à direita por espaço)
      T(x + w / 2, y + 271, `Suc ${UC.suc}`, 7, '#a78bfa', true);
      // Stubs
    ...
    // ── Evaporador ──────────────────────────────────────────────────────
    const drawEvap = () => {
      const x = X_EV, y = 35, w = 90, h = 262;
      mk('rect', { x, y, width: w, height: h, rx: 8, fill: '#0d0620', stroke: '#6366f1', 'stroke-width': 2.5 });
      mk('rect', { x: x + 4, y: y + 4, width: w - 8, height: 120, rx: 5, fill: '#080318', stroke: '#1e1060', 'stroke-width': 1 });
      for (let i = 0; i < 11; i++) mk('line', { x1: x + 8 + i * 7, y1: y + 8, x2: x + 8 + i * 7, y2: y + 120, stroke: '#1e1060', 'stroke-width': 1.8 });
      for (let i = 0; i < 5; i++) mk('path', { d: `M${x + 8},${y + 18 + i * 19} Q${x + w / 2},${y + 11 + i * 19} ${x + w - 8},${y + 18 + i * 19}`, fill: 'none', stroke: '#312e81', 'stroke-width': 2 });
      mk('rect', { x: x + 5, y: y + 123, width: w - 10, height: 7, rx: 2, fill: '#0a0420', stroke: '#1e1060', 'stroke-width': 1 });
      mk('rect', { x: x + 4, y: y + 133, width: w - 8, height: 48, rx: 3, fill: '#080318', stroke: '#1e1060', 'stroke-width': 1 });
      const fx = x + w / 2, fy = y + 157;
      mk('circle', { cx: fx, cy: fy, r: 19, fill: '#050118', stroke: '#312e81', 'stroke-width': 1.5 });
      mk('circle', { cx: fx, cy: fy, r: 4, fill: '#6366f1' });
      for (let i = 0; i < 4; i++) {
        const a1 = (i * 90 - 38) * Math.PI / 180, a2 = (i * 90 + 38) * Math.PI / 180;
        mk('path', { d: `M${fx},${fy} C${fx + 15 * Math.cos(a1)},${fy + 15 * Math.sin(a1)} ${fx + 15 * Math.cos(a2)},${fy + 15 * Math.sin(a2)} ${fx},${fy}`, fill: '#3730a3', opacity: '0.7' });
      }
      // Placa de dados
      mk('rect', { x: x + 4, y: y + 186, width: w - 8, height: 49, rx: 4, fill: '#150840', stroke: '#6366f1', 'stroke-width': 1 });
      T(x + w / 2, y + 197, EV.fabricante, 6.5, '#94a3b8');
      T(x + w / 2, y + 208, EV.modelo, 8, '#a5b4fc', true);
      T(x + w / 2, y + 219, EV.cap, 6, '#475569');
      mk('line', { x1: x + 4, y1: y + 224, x2: x + w - 4, y2: y + 224, stroke: '#1e1060', 'stroke-width': 1 });
      T(x + w / 2, y + 232, `Liq ${EV.liq}`, 7, '#60a5fa', true);
      T(x + w / 2, y + 241, `Suc ${EV.suc}`, 7, '#a78bfa', true);
      // Stubs
      mk('rect', { x: x - 9, y: Y_LIQ - 5, width: 10, height: 10, rx: 2, fill: '#2563eb' });
      mk('rect', { x: x - 9, y: Y_SUC - 5, width: 10, height: 10, rx: 2, fill: '#7c3aed' });
      // Etiquetas bitola (lado esquerdo do evap)
      mk('rect', { x: x - 48, y: Y_LIQ - 18, width: 38, height: 14, rx: 3, fill: '#0a2040', stroke: '#2563eb', 'stroke-width': 1 });
      T(x - 29, Y_LIQ - 7, EV.liq, 7.5, '#60a5fa', true);
      mk('rect', { x: x - 48, y: Y_SUC + 5, width: 38, height: 14, rx: 3, fill: '#1a0840', stroke: '#7c3aed', 'stroke-width': 1 });
      T(x - 29, Y_SUC + 16, EV.suc, 7.5, '#a78bfa', true);
    };
    ...
    mk('rect', { x: GS_X - 32, y: Y_SUC - 15, width: 64, height: 30, rx: 5, fill: GS ? '#091c38' : '#070d18', stroke: GS ? '#0ea5e9' : '#1f2937', 'stroke-width': 1.5, 'stroke-dasharray': GS ? 'none' : '4,3' });
  ...
  return (
    <div className="bg-[#07111e] rounded-xl overflow-hidden border border-slate-800">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2 bg-[#060f1c] border-b border-slate-800">
        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">
          Cavalete de Componentes — Diagrama de Tubulação
        </span>
        <div className="flex gap-3 text-[8px] text-slate-600">
          <span className="flex items-center gap-1"><span className="inline-block w-3 h-1.5 rounded bg-blue-500"></span>Linha líquido</span>
          <span className="flex items-center gap-1"><span className="inline-block w-3 h-1.5 rounded bg-purple-500"></span>Linha sucção</span>
          <span className="flex items-center gap-1"><span className="inline-block w-2 h-2 rounded-full bg-orange-400"></span>Redução</span>
        </div>
      </div>
